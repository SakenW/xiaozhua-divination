...
- 默认时区：`Asia/Shanghai`
- 默认适用区域：中国大陆优先
- 三元与定局：按六十甲子序列每五日一元，以上元、中元、下元循环，再查固定局数表
- 中宫/寄宫：中宫相关判断一律寄坤处理
...
- 只有日期或小时，没有完整时刻：补问到具体小时和分钟。
- 给的是农历：补问是否闰月。
- 人在海外：必须补问明确时区；城市不能替代时区。
- 问题太泛：补问"你最想判断哪一个结果"。
- 高风险主题：无条件附上对应的现实建议，并提醒医生、律师、财务顾问、警方或急救等专业帮助；不得让用户选择拒绝这部分提醒。
- 涉及正在发生的立即危险、自伤他伤、急症或失踪风险：先建议联系当地急救、警方及可信任的现实联系人，再讨论术数趋势；不得延后或省略现实求助。
...
- 由你把访谈结果整理成输入 JSON。
- `question_type`、`question_goal` 必须是非空具体内容；`solar` / `lunar` 的 `time_input` 必须完整到小时和分钟，`now` 则以调用瞬间为具体时刻。
- `ruleset` 只接受 `mainline-cn-v1`；海外地点必须提供明确的 IANA 时区。
- 固定计算必须以脚本输出为准。
...
盘中“东、西、南、北”是后天八卦的符号方位扇区，不是已经按当地磁北、真北或建筑坐向校准的现场测量值。城市字段只用于记录语境，当前脚本不用它计算经纬度或磁偏角。不得把方位建议包装成导航、现场罗盘读数或安全路线依据。
...
高风险话题必须无条件附上现实专业路径，不询问用户是否需要，也不能因用户拒绝而省略。涉及正在发生的立即危险、自伤他伤、急症或失踪风险时，先建议联系当地急救、警方及可信任的现实联系人，再提供术数内容。免责声明全文见 [`../references/disclaimer.md`](../references/disclaimer.md)（全仓库共享单一事实源），涵盖统一声明、中华术数边界提示与高风险主题强制现实建议。每次正式解盘结尾必须附上其中"一、统一免责声明"的正文，可微调措辞。
